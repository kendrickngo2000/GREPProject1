...
#include <string.h>
char Q[] = "", char T[]= "TMP";
int	peekc, lastc, given, io, pflag, ninbuf, names[26], anymarks;
char savedfile[FNSIZE], file[FNSIZE], linebuf[LBSIZE], rhsbuf[LBSIZE / 2], expbuf[ESIZE + 4], genbuf[LBSIZE], * nextip, * linebp, * globp;
unsigned int *addr1, *addr2, *dot, *dol, *zero;
long count, lseek(int, long, int);
int	open(char *, int), creat(char *, int), read(int, char*, int), write(int, char*, int), close(int), fork(void), execl(char *, ...), exit(int), wait(int *), unlink(char *);
int	vflag = 1, oflag, listf, listn, col, tfile = -1, tline;
char* tfname, * loc1, * loc2, ibuff[BLKSIZE], obuff[BLKSIZE];
int	iblock	= -1, oblock = -1, int ichanged, nleft;
char WRERR[] = "WRITE ERROR", char *braslist[NBRA], *braelist[NBRA];
int	nbra, subnewa, subolda, fchange, wrapp, bpagesize = 20;
unsigned nlall = 128;
char *mktemp(char *), tmpXXXXX[50] = "/tmp/eXXXXX", *malloc(int), *realloc(char *, int), char *getblock(unsigned int atl, int iof), *getline(unsigned int tl), *place(char *sp, char *l1, char *l2);
void add(int i);
int advance(char *lp, char *ep), append(int (*f)(void), unsigned int* a), backref(int i, char* lp), cclass(char* set, int c, int af), compsub(void);
void blkio(int b, char *buf, int (*iofcn)(int, char*, int)), void callunix(void), commands(void), compile(int eof), dosub(void), error(char *s), void exfile(void), filename(int comm), gdelete(void);
int execute(unsigned int* addr); getchr(void), getcopy(void), getfile(void), getnum(void), getsub(void), gettty(void), gety(void), putline(void);
void global(int k), init(void);
unsigned int *address(void);
void join(void), move(int cflag), newline(void), nonzero(void), onhup(int n), onintr(int n), print(void), putchr(int ac), putd(void), putfile(void), puts(char *sp), quit(int n), rdelete(unsigned int *ad1, unsigned int *ad2);
void reverse(unsigned int *a1, unsigned int *a2),setwide(void), setnoaddr(void), squeeze(int i), substitute(int inglob);
jmp_buf	savej;
typedef void (*SIG_TYP)(int);
SIG_TYP	oldhup, oldquit;
int main(int argc, char *argv[]) { char *p1, *p2; SIG_TYP oldintr;
	...
	argv++;
	while (argc > 1 && **argv=='-') {switch((*argv)[1]) { case '\0': vflag = 0; break; case 'q': signal(SIGQUIT, SIG_DFL); vflag = 1; break; case 'o': oflag = 1; break; }
		argv++; argc--;}
	if (oflag) {p1 = "/dev/stdout"; p2 = savedfile; while (*p2++ = *p1++); }
	if (argc>1) {p1 = *argv;p2 = savedfile; while (*p2++ = *p1++)if (p2 >= &savedfile[sizeof(savedfile)]) p2--; globp = "r"; }
...
unsigned int *address(void) {int sign; unsigned int *a, *b; int opcnt, nextopand; int c; nextopand = -1; sign = 1; opcnt = 0; a = dot;
	do { do c = getchr(); while (c==' ' || c=='\t');
		...
		default: if (nextopand == opcnt) { a += sign; if (a<zero || dol<a) continue;  /* error(Q); */}
			if (c!='+' && c!='-' && c!='^') { peekc = c; if (opcnt==0) a = 0; return (a); } sign = 1;
...
void newline(void) { int c;
	if ((c = getchr()) == '\n' || c == EOF) return;
	if (c=='p' || c=='l' || c=='n') {pflag++;if (c=='l')listf++;else if (c=='n') listn++;if ((c=getchr())=='\n') return; }error(Q); }
void filename(int comm) { char *p1, *p2; int c; count = 0; c = getchr();
	...
	while ((c = getchr()) == ' ');
	if (c=='\n') error(Q); p1 = file;
	do { if (p1 >= &file[sizeof(file)-1] || c==' ' || c==EOF) error(Q); *p1++ = c;} 
	while ((c = getchr()) != '\n'); *p1++ = 0;
	if (savedfile[0]==0 || comm=='e' || comm=='f') {p1 = savedfile; p2 = file; while (*p1++ = *p2++);} }
void exfile(void) {close(io);io = -1; 
if (vflag) {putd(); putchr('\n');}}
void onintr(int n) {signal(SIGINT, onintr); putchr('\n'); lastc = '\n'; error(Q);}
void onhup(int n) { signal(SIGINT, SIG_IGN);
	signal(SIGHUP, SIG_IGN);
	if (dol > zero) {addr1 = zero+1;addr2 = dol;io = creat("ed.hup", 0600);if (io > 0) putfile();} fchange = 0; quit(0);}
void error(char *s) { int c; wrapp = 0; listf = 0; listn = 0; putchr('?'); puts(s); count = 0; lseek(0, (long)0, 2); pflag = 0;
	if (globp) lastc = '\n'; globp = 0; peekc = lastc;
	if(lastc)
		while ((c = getchr()) != '\n' && c != EOF);
...
int getchr(void) { char c;
	...
	if (globp) { 
		if ((lastc = *globp++) != 0)
			return(lastc); globp = 0; return(EOF); }
	...
	int gety(void) { int c; char* gf; char* p; p = linebuf; gf = globp;
		while ((c = getchr()) != '\n') {
			if (c == EOF) { if (gf) peekc = c; return(c); }
			if ((c &= 0177) == 0) continue; *p++ = c;
			if (p >= &linebuf[LBSIZE - 2]) error(Q); } *p++ = 0;return(0); }
	...
	void putfile(void) { unsigned int* a1; int n; char* fp, * lp; int nib; nib = BLKSIZE; fp = genbuf; a1 = addr1;
		do { lp = getline(*a1++);
			for (;;) {
				if (--nib < 0) { n = fp - genbuf; if (write(io, genbuf, n) != n) { puts(WRERR); error(Q); } nib = BLKSIZE - 1; fp = genbuf; } count++;
				if ((*fp++ = *lp++) == 0) { fp[-1] = '\n'; break; } }
	...
	int append(int (*f)(void), unsigned int* a) {
		...
		while ((*f)() == 0) {
			if ((dol - zero) + 1 >= nlall) {
				...
				if ((zero = (unsigned*)realloc((char*)zero, nlall * sizeof(unsigned))) == NULL) { error("MEM?"); onhup(0); }dot += zero - ozero; dol += zero - ozero; }
...
void callunix(void) {SIG_TYP savint;int pid, rpid;int retcode; setnoaddr();
...
void rdelete(unsigned int *ad1, unsigned int *ad2) { unsigned int *a1, *a2, *a3; a1 = ad1; a2 = ad2+1; a3 = dol; dol -= a2 - a1;
	do {*a1++ = *a2++;} 
	while (a2 <= a3); a1 = ad1;
	if (a1 > dol) a1 = dol; dot = a1; fchange = 1; }
void gdelete(void) { unsigned int *a1, *a2, *a3; a3 = dol;
	for (a1=zero; (*a1&01)==0; a1++) if (a1>=a3) return;
	for (a2=a1+1; a2<=a3;) { if (*a2&01) {a2++; dot = a1;} else *a1++ = *a2++;} dol = a1-1;
	if (dot>dol) dot = dol; fchange = 1; }
char * getline(unsigned int tl) { char *bp, *lp; int nl; lp = linebuf; bp = getblock(tl, READ); nl = nleft; tl &= ~((BLKSIZE/2)-1);
	while (*lp++ = *bp++)
...
int putline(void) {	char *bp, *lp; int nl; unsigned int tl;
	...
	while (*bp = *lp++) { if (*bp++ == '\n') {*--bp = 0; linebp = lp; break; }
		if (--nl == 0) { bp = getblock(tl+=(BLKSIZE/2), WRITE); nl = nleft;}} nl = tline; tline += (((lp-linebuf)+03)>>1)&077776; return(nl); }
char * getblock(unsigned int atl, int iof) { int bno, off; bno = (atl/(BLKSIZE/2)); off = (atl<<1) & (BLKSIZE-1) & ~03;
	...
	if (iof==READ) { 
		if (ichanged) blkio(iblock, ibuff, write); ichanged = 0; iblock = bno; blkio(bno, ibuff, read);
		return(ibuff+off);}
	if (oblock>=0) blkio(oblock, obuff, write); oblock = bno;
	return(obuff+off);}
...
void global(int k) {char *gp; int c; unsigned int *a1; char globuf[GBSIZE];
	...
	if (a1>=addr1 && a1<=addr2 && execute(a1)==k) *a1 |= 01; } /* Special case: g/.../d (avoid n^2 algorithm */
...
void join(void) {char *gp, *lp;unsigned int *a1;nonzero();gp = genbuf;
	for (a1=addr1; a1<=addr2; a1++) {lp = getline(*a1);
		while (*gp = *lp++)
			if (gp++ >= &genbuf[LBSIZE-2]) error(Q);} lp = linebuf; gp = genbuf;
	while (*lp++ = *gp++); *addr1 = putline();
	if (addr1<addr2) rdelete(addr1+1, addr2); dot = addr1;}
void substitute(int inglob) { int *mp, nl; unsigned int *a1; int gsubf; int n; n = getnum();	/* OK even if n==0 */
	gsubf = compsub();
	for (a1 = addr1; a1 <= addr2; a1++) {if (execute(a1)) {unsigned *ozero;int m = n;
			...
			if (m <= 0) {inglob |= 01; subnewa = putline(); *a1 &= ~01;
				if (anymarks) { for (mp = names; mp < &names[26]; mp++) 
					if (*mp == *a1) *mp = subnewa;} subolda = *a1; *a1 = subnewa;ozero = zero; nl = append(getsub, a1); nl += zero-ozero; addr2 += nl;}}}
	if (inglob==0) error(Q);}
int compsub(void) { int seof, c; char *p; 
	...
	if (c=='\\') c = getchr() | 0200;
		...
		if (c==seof) break; *p++ = c;
		if (p >= &rhsbuf[LBSIZE/2]) error(Q);} *p++ = 0;
...
void dosub(void) { char *lp, *sp, *rp; int c; lp = linebuf; sp = genbuf; rp = rhsbuf;
	...
	while (c = *rp++&0377) {
		...
		if (sp >= &genbuf[LBSIZE]) error(Q); } lp = loc2; loc2 = sp - genbuf + linebuf;
	while (*sp++ = *lp++)
		if (sp >= &genbuf[LBSIZE]) error(Q); lp = linebuf; sp = genbuf;
	while (*lp++ = *sp++) ;}
char * place(char *sp, char *l1, char *l2) {
	while (l1 < l2) { *sp++ = *l1++;
		if (sp >= &genbuf[LBSIZE]) error(Q); } return(sp); }
...
void compile(int eof) {
	...
	for (;;) {
		...
		case '[':  *ep++ = CCL;  *ep++ = 0;  cclcnt = 1;  if ((c = getchr()) == '^') { c = getchr();  ep[-2] = NCCL; }
			...
			defchar:  default:  *ep++ = CCHR;  *ep++ = c; } }  cerror:  expbuf[0] = 0;  nbra = 0;  error(Q);}
int execute(unsigned int *addr) { char *p1, *p2; int c;
	for (c=0; c<NBRA; c++) { braslist[c] = 0; braelist[c] = 0; } p2 = expbuf;
	if (addr == (unsigned *)0) { if (*p2==CCIRC) return(0);p1 = loc2; } else if (addr==zero) return(0);
	else p1 = getline(*addr);
	if (*p2==CCIRC) { loc1 = p1; return(advance(p1, p2+1)); } /* fast check for first character */
	if (*p2==CCHR) { c = p2[1];
		do { if (*p1!=c) continue;
...
int backref(int i, char *lp) { char *bp; bp = braslist[i];
	while (*bp++ == *lp++)
		if (bp >= braelist[i]) return(1); return(0);}
...
void puts(char *sp) { col = 0;
	while (*sp)putchr(*sp++); putchr('\n'); }
char line[70];
char *linp	= line;
